...
export function StudentUploadImage(){
    ...
    axios.post(BASE_URL + "/add-temp-image", fd)
      .then((response) => {
        console.log(response);
        window.alert("Image is uploaded.. it will displayed on screen after approval...")
        navigate('/gallery')
      })
      .catch((error) => {
        if (error.response) {
...
<br />
            ...
            <div className="row align-items-center py-3">
                    <div class="col-md-3 ps-5">
                      ...
                      <input type="text" name="description" onChange={handleForm} className="form-control form-control-lg" />     
                      </div>
                    </div>
            </div>
            <hr className="mx-n3" />
