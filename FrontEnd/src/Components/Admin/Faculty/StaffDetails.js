...
export function StaffDetails(){
  ...
  const navigate  = useNavigate()
    ...
    function getStaff(){
        axios.get(BASE_URL + '/staff').then((res)=>{
    ...
    const handleDelete = (el) => {    
      let uc  =  window.confirm("Do you really want to delete Staff data ? ")
            if(uc == true)
            {
...
<button type="button" className="btn btn-primary btn-lg" style={{borderRadius: "5px",marginTop: "20px"}}><NavLink to="/admin/addstaff">Add Staff</NavLink></button> <br /><br />
  <div className="row">
